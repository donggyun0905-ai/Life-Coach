from fastapi import APIRouter, Depends, HTTPException, Security, File, UploadFile, Header, Form
from sqlalchemy.orm import Session
from typing import List
from fastapi.security import HTTPAuthorizationCredentials, HTTPBearer
from datetime import date
from ..models.pill import PillResult
from ..models.pill_ai import analyze_pill_image_llm
import uuid
import os, shutil
import json
...
async def ingest_pill_image(
    uid: str = Form(...),                   # 프론트에서 form-data 로 넘김
    ...
    # 4) LLM 호출해서 분석
    llm_result = analyze_pill_image_llm(save_path)

    # 만약 API 키 문제나 오류면 바로 반환 (DB는 안 남기고)
    ...
    pill = PillResult(
        uid=uid,
        image_path=save_path,
        pill_name=llm_result.get("pill_name"),
    ...
    return {
        ...
        "id": pill.id,
        "uid": pill.uid,
        "image_path": pill.image_path,
        "pill_name": pill.pill_name,
        "appearance": pill.appearance,
        "main_usage": pill.main_usage,
